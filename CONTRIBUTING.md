# Contributing to ep_cursortrace

Thanks for helping improve `ep_cursortrace` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_cursortrace` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_cursortrace from your local clone
pnpm run plugins i --path ../ep_cursortrace

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_cursortrace/` are picked up after a server restart.
...
* Etherpad version, ep_cursortrace version, browser + OS
* Steps to reproduce
